...
const getAllIdeaWithFilter = async (filter = filterEnum.ALPHABET, page = 1, limit = 5) => {
  switch (filter) {
    ...
    case filterEnum.LIKE:
      const allPostWithLike = await IdeaModel.find({}).populate('category');
    ...
    case filterEnum.DISLIKE:
        const allPostWithDislike = await IdeaModel.find({}).populate(
          "category"
        );
...
module.exports = { getFileUrl, createIdea, createDocumentFromMarkdown, getAllIdeaWithFilter, countAllIdeas };
